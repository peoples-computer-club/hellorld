...
uart2_init:
  ...
  // And also configure PA2 and PA3 for use by the USART2. This is done with the GPIOA
  // registers (base 0x4001 0800). We need to set PA2 for alternate function push-pull
  // output and PA3 to input mode with pull-up/pull-down. This means putting 00101011 in
  // bits 8-15 of GPIOA_CRL.
  ldr r0, =0x40010800
  ldr r1, [r0]
  // Clear bits 8-15
  bic r1, r1, #0xFF00
  // And then set the 1's in bits 8-15
  orr r1, r1, #0x2B00
  // Store back
  str r1, [r0]
...
uart2_tx_chr:
  ...
  ldr r1, =0x40004400
  // We store bit 6 as 1 in r3 so we can compare with the register contents
uart2_tx_wait:
